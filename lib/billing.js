...
function sessionPrice(db, group) {
  ...
  if (!branch) return 1000;
  return group.durationMin === 90 ? Number(branch.priceHourHalf) || 1500 : Number(branch.priceHour) || 1000;
}
...
function childSpent(db, child, month) {
  ...
  for (const gid of childGroupIds(child)) {
    ...
    for (const day of month.days.map((d) => d.day)) {
      if (isSick(db, child.id, month, day) && planned.includes(day)) {
        sickCount += 1;
...
function periodCore(db, familyId, month) {
  ...
  const disc = familyDiscount(regular.length);
  let advanceRaw = 0;
  let spent = 0;
  let sickCredit = 0;
  const formula = [];
