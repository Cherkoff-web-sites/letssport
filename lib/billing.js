...
function childBilling(db, child, month) {
  ...
  const toPayLessons = Math.max(0, plannedCount - excused);
  const toPaySum = money(toPayLessons * lessonPrice);
  const size = db.children.filter((c) => c.familyId === child.familyId).length;
  const autoDiscount = familyDiscount(size);
  const discountPercent = child.discountPercent == null ? autoDiscount : Number(child.discountPercent);
