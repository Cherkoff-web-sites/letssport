...
const BRANCH_ORDER = ["ТИМИРЯЗЕВСКАЯ", "ВАЛДАЙСКИЙ", "ФЕСТИВАЛЬНАЯ", "СЕРЕБРЯНЫЙ", "БЕГОВАЯ"];
...
function migrate(db) {
  ...
  const hgGroups = (db.groups || []).filter((g) => g.sport === "hg").map((g) => g.id);
  const samboGroups = (db.groups || []).filter((g) => g.sport === "sambo").map((g) => g.id);
  db.trainers = [
    { id: "t1", name: "Тренер ХГ", sport: "hg", login: "hg", password: "hg", groupIds: hgGroups },
    { id: "t2", name: "Тренер Самбо", sport: "sambo", login: "sambo", password: "sambo", groupIds: samboGroups }
  ];
